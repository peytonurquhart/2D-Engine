...
public:
	...
	bool isEmpty() { if (chunk == nullptr) { return true; } return false; }
	...
	//will translate an amount of positive or negative chunksizes x and y
	...
	//returns true if one of the boxes is touching the chunk only including blocks that are not null
	bool isTouchingChunk(Hitbox *boxes, int numBoxes);

	//returns the ratio that the speed should be divided by if there is room to move less than speed (usually 1)
	bool willBeTouchingChunk(Hitbox *boxes, int numBoxes, float speed, const string &direction, int numMovesInDirection);
